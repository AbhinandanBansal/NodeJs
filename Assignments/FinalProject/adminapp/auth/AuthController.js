...
router.post('/register', function(req, res) {
    ...
    const hashedPassword = bcrypt.hashSync(req.body.passwordNewUser, 8);
    var userType;
    ...
    User.create({
      name : req.body.nameNewUser,
      email : req.body.emailNewUser,
...
router.post('/login', function(req, res) {
    User.findOne({ email: req.body.emailLogin }, function (err, user) {
      if (err) return res.status(500).send('Error on the server.');
      const string = encodeURIComponent('! Please enter valid value');
      if (!user) { res.redirect('/?valid=' + string);}
      else{
       
        const passwordIsValid = bcrypt.compareSync(req.body.passwordLogin, user.password);
        if (!passwordIsValid) return res.status(401).send({ auth: false, token: null });
        var token = jwt.sign({ id: user._id,userType: user.userType,name: user.name, email: user.email }, config.secret, {
            expiresIn: 86400 // expires in 24 hours
        });
        localStorage.setItem('authtoken', token)
        if(user.userType === 1)
        {
          //res.redirect(`/news/createnews`,);
        res.render('addNews.ejs',{data:user,msg:'',error:''});
        }
      }
    });
});
